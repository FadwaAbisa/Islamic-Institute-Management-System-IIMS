...
import { PrismaClient, MaritalStatus } from "@prisma/client";
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            try {
                ...
                const staffData = {
                    ...
                    // البيانات الاختيارية
                    birthday: row['تاريخ الميلاد (YYYY-MM-DD)'] ? parseDate(row['تاريخ الميلاد (YYYY-MM-DD)'].toString()) || new Date('1990-01-01') : new Date('1990-01-01'),
                    address: row['عنوان السكن']?.toString().trim() || null,

                    // البيانات الوظيفية
                    appointmentDate: row['تاريخ التعيين (YYYY-MM-DD)'] ? parseDate(row['تاريخ التعيين (YYYY-MM-DD)'].toString()) : undefined,
                    serviceStartDate: row['تاريخ بداية الخدمة (YYYY-MM-DD)'] ? parseDate(row['تاريخ بداية الخدمة (YYYY-MM-DD)'].toString()) : undefined,
...
function convertMaritalStatus(status: string): MaritalStatus {
    const statusMap: { [key: string]: MaritalStatus } = {
    ...
    return statusMap[status.trim()] || MaritalStatus.SINGLE;
}
