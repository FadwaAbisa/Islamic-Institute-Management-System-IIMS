...
const MessagesPage = () => {
  ...
  const fetchAvailableUsers = async (search: string = "") => {
    try {
      const response = await fetch(`/api/users?userType=${userType}&search=${search}`);
      if (response.ok) {
        const data = await response.json();
            ...
            <div className="p-6">
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {availableUsers.map((user) => (
                  <Link
                    key={user.id}
                    href={`/list/messages/new?receiverId=${user.id}&receiverType=${user.type}&receiverName=${encodeURIComponent(user.fullName)}`}
