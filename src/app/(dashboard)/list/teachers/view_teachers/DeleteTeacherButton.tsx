...
export default function DeleteTeacherButton({ teacherId, teacherName = "المعلم" }: DeleteTeacherButtonProps) {
  ...
  const handleDelete = async () => {
    if (!showConfirm) {
      ...
      showToast({
        type: 'warning',
        title: 'تأكيد الحذف',
        description: `هل أنت متأكد من حذف ${teacherName}؟ هذا الإجراء لا يمكن التراجع عنه.`
      })
      return
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast({
          type: 'error',
          title: 'فشل في الحذف',
          description: errorData.message || 'حدث خطأ أثناء حذف المعلم. يرجى المحاولة مرة أخرى.'
        })
      }
    } catch (error) {
      showToast({
        ...
        description: 'حدث خطأ في الاتصال بالخادم. تحقق من اتصال الإنترنت وحاول مرة أخرى.'
      })
    } finally {
      setIsDeleting(false)
    }
  ...
  const handleCancel = () => {
    ...
    showToast({
      type: 'info',
      title: 'تم الإلغاء',
      description: 'تم إلغاء عملية الحذف بنجاح.'
    })
  }
  ...
  const handleUndo = async () => {
    ...
    try {
      ...
      showToast({
        type: 'info',
        title: 'استعادة المعلم',
        description: 'سيتم إضافة منطق استعادة المعلم قريباً.'
      })
    } catch (error) {
      showToast({
        type: 'error',
        title: 'خطأ في الاستعادة',
        description: 'حدث خطأ أثناء محاولة استعادة المعلم.'
      })
    }
  }
